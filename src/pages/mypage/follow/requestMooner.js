        ...
        if (token) {
            ...
            axios.get('https://api.usdiary.site/friends/follow-request/handle', {
                ...
                .then((response) => {
                    ...
                    setEntireUsers(response.data.data);
                })
                .catch((error) => {
    ...
    const handleAccept = async (user) => {
        ...
        try {
            const response = await axios.post('https://api.usdiary.site/friends/follow-request/handle', {
                ...
                status: "accepted",        // 수락 액션
            }, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,  // 헤더에 토큰 포함
                }
            });
                ...
                <div className='request_box'>
                    {entireUsers.map((user, index) => (
                        <div key={index} className='profile-follow_box_content_box_friend'>
                            <img
                                src={user.follower_profile_img  || basicprofileimg}
                                className='profile-follow_box_content_box_friend_img'
                                alt='profile'
                            />
                            <div className='profile-follow_box_content_box_friend_text'>
                                <div className='profile-follow_box_content_box_friend_text_nickname'>{user.follower_user_nick}</div>
                                <div className='profile-follow_box_content_box_friend_text_id'>{user.follower_sign_id}</div>
                            </div>
